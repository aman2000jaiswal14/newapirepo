...
'''
# suggested logic by ai studio
   # Inside split_logic class
from decimal import Decimal

def simplify_debts(group_data, all_items):
    """
    Industry Grade Debt Simplification.
    1. Calculates Net Balance for every member.
    2. Minimizes transactions using a greedy matching algorithm.
    """
    members = group_data.get('groupMembers', [])
    # Initialize all balances to zero
    # Use Decimal for 100% precision
    net_balances = {uid: Decimal('0') for uid in members}

    # STEP 1: Calculate Global Net Balance from all items
    for item in all_items:
        # Payer gets a positive balance (they spent money)
        payer_id = item.get('itemPayer')[0]
        total_amt = Decimal(str(item.get('itemTotalAmount', 0)))
        if payer_id in net_balances:
            net_balances[payer_id] += total_amt
        
        # Splitters get a negative balance (they owe money)
        splitters = item.get('itemSpliter', [])
        values = item.get('itemSpliterValue', [])
        for i, s_id in enumerate(splitters):
            if s_id in net_balances:
                net_balances[s_id] -= Decimal(str(values[i]))
    ...
    for uid, balance in net_balances.items():
        if balance < 0:
            debtors.append({'id': uid, 'bal': abs(balance)})
        elif balance > 0:
            creditors.append({'id': uid, 'bal': balance})

    # STEP 3: Greedy Matching to minimize transactions
    # New empty graph
    new_graph = {uid: {} for uid in members}
    ...
    while d_idx < len(debtors) and c_idx < len(creditors):
        ...
        settle_amt = min(debtor['bal'], creditor['bal'])
        
        # Update graph: Debtor owes Creditor
        if settle_amt > 0:
            # We store as float for Firebase compatibility
            new_graph[creditor['id']][debtor['id']] = float(settle_amt)
        
        # Update remaining balances
        debtor['bal'] -= settle_amt
        creditor['bal'] -= settle_amt
        
        if debtor['bal'] == 0: d_idx += 1
        if creditor['bal'] == 0: c_idx += 1

    # Overwrite the old groupGraph with the optimized one
...
def recalculate_group_balances(self, group_id):
    ...
    def transaction_handler(current_group):
        if current_group is None: return None
        
        # 1. Fetch all actual item data for this group
        item_ids = current_group.get("groupItems", [])
        all_items = []
        for iid in item_ids:
            item_data = db.reference(f"items/{iid}").get()
            if item_data:
                all_items.append(item_data)
        
        # 2. Run the Simplification Logic
        from split_logic import simplify_debts
        simplify_debts(current_group, all_items)
        
        return current_group

    group_ref.transaction(transaction_handler)
...
def create_item_atomically(self, item_data):
    ...
    item_data["payerName"] = user.get("name", "Unknown") if user else "Unknown"
    
    db.reference(f"items/{item_id}").set(item_data)

    # 2. Add ID to group and trigger recalculation
    group_id = item_data["itemGroupId"]
    group_ref = db.reference(f"groups/{group_id}")
    
    def add_to_group_transaction(group):
        if group:
            group.setdefault("groupItems", []).append(item_id)
        return group
    
    group_ref.transaction(add_to_group_transaction)
    
    # 3. 🔥 Run the Global Simplification
    self.recalculate_group_balances(group_id)
